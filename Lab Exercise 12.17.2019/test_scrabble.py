...
def test_isValidWord(wordList):
    ...
    if not isValidWord(word, handCopy, wordList):
        ...
        if handCopy != handOrig:
            ...
            print ("The word", word, "should be in wordList - is it?",
                   wordInWL)
    ...
    if  isValidWord(word, hand, wordList):
        ...
        print ("\tExpected False, but got True for word: '" +
               word + "' and hand:", hand)
        print ("\t(If this is the only failure, make sure " +
               "isValidWord() isn't mutating its inputs)" )       
        
        failure = True        

    if not failure:
        print ("SUCCESS: test_isValidWord()")
#end of test_validValidWord
